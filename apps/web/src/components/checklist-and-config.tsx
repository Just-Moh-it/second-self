...
import type { PhoneNumber } from '@/components/types';
  ...
  useEffect(() => {
    ...
    const pollChecks = async () => {
      try {
        ...
        // Error handling removed console.error as per ultracite rules
      }
    };
    ...
    return [
        ...
        field:
          phoneNumbers.length > 0 ? (
            phoneNumbers.length === 1 ? (
              <Input disabled value={phoneNumbers.at(0)?.friendlyName || ''} />
            ) : (
              <Select
                onValueChange={(value) => {
      ...
      },
      {
        ...
        field: (
          <div className="flex w-full items-center gap-2">
            <div className="flex-1">
              <Input className="w-full" disabled value={currentVoiceUrl} />
            </div>
            <div className="flex-1">
              <Button
                className="w-full"
      ...
      <DialogContent className="w-full max-w-[800px]">
        ...
        <div className="mt-4 space-y-0">
          {checklist.map((item, i) => (
            <div
              className="grid grid-cols-1 gap-x-6 py-2 sm:grid-cols-2"
              key={i}
            >
              ...
              <div className="mt-2 flex items-center sm:mt-0">{item.field}</div>
            </div>
          ))}
        </div>
